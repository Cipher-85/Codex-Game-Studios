...
If [B]: display the complete proposed systems-index section, then re-ask with `numbered choice prompt`.
...
Use `numbered choice prompt` for the write approval:
...
For `rtm` mode, use `numbered choice prompt`:
