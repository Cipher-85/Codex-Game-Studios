---
name: asset-spec
description: "Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after art bible and GDD/level design are approved, before production begins for Codex Game Studios."
---

If no argument is provided, check whether `design/assets/entity-inventory.md` exists:
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `numbered choice prompt`:
...
Present the full proposed inventory to the user in conversation. Then use `numbered choice prompt`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `numbered choice prompt`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `numbered choice prompt`:
...
Present the full identified list to the user. Use `numbered choice prompt`:
...
Spawn specialist agents based on review mode. **Issue all Codex subagent delegation calls simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** via Codex subagent delegation:
...
**`technical-artist`** via Codex subagent delegation:
...
After presenting all specs, use `numbered choice prompt`:
