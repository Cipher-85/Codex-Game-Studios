...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `numbered choice prompt`** at key decision points throughout brainstorming:
   ...
   `numbered choice prompt` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an numbered choice prompt with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `numbered choice prompt` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `numbered choice prompt` with these exact tab labels:
...
Present all three. Then use `numbered choice prompt` to capture the selection.
...
Ask these as `numbered choice prompt` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `numbered choice prompt`:
...
If the user selects B, C, or D, make the revision, then use `numbered choice prompt` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via Codex subagent delegation in parallel before moving to Phase 5. Issue both Codex subagent delegation calls simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-tab `numbered choice prompt`:
...
- **Target platform**: Use `numbered choice prompt` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `numbered choice prompt` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via Codex subagent delegation using gate TD-FEASIBILITY (`.codex/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Codex subagent delegation using gate PR-SCOPE (`.codex/docs/director-gates.md`).**
...
5. Use `numbered choice prompt` for write approval:
...
If [B]: ask which section to revise using `numbered choice prompt` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `numbered choice prompt` — "Ready to write the updated concept document?"
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, WebSearch, Codex subagent delegation, numbered choice prompt`
- `model: sonnet`
