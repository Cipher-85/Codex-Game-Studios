---
name: code-review
description: "Performs an architectural and quality code review on a specified file or set of files. Checks for coding standard compliance, architectural pattern adherence, SOLID principles, testability, and performance concerns for Codex Game Studios."
---
...
Spawn all applicable specialists simultaneously via Codex subagent delegation — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` via Codex subagent delegation in parallel with the engine specialists. Pass:
...
- If verdict is CHANGES REQUIRED or MAJOR REVISION, the best next action is normally
  to fix the required issues, then re-run `$code-review`.
- If an equally plausible lane exists (for example a sprint smoke check is owed
  or the user is about to stop), use `request_user_input` when available with
  2-3 choices. If unavailable, use a concise numbered prompt with the same lanes.
- Suggest `$handoff` only when installed and session state should be preserved.
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Bash, Codex subagent delegation, numbered choice prompt`
- `model: sonnet`
