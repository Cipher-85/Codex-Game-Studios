...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `numbered choice prompt`:
...
**Step 2 — Spawn `lead-programmer` via Codex subagent delegation using gate LP-FEASIBILITY (`.codex/docs/director-gates.md`):**
...
Use `numbered choice prompt` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `numbered choice prompt`:
...
5. **Use `numbered choice prompt` for write approvals** — plain text "May I?" is not
