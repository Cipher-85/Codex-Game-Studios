...
Spawn `technical-director` via Codex subagent delegation using gate **TD-MANIFEST** (`.codex/docs/director-gates.md`).
...
- **CONCERNS** → surface via `numbered choice prompt` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, Codex subagent delegation`
- `model: sonnet`
