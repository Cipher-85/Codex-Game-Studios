...
Use `numbered choice prompt` to confirm proceeding or reduce scope.
...
Spawn `release-manager` via Codex subagent delegation. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` via Codex subagent delegation with:
...
3. Spawn `qa-tester` via Codex subagent delegation to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` via Codex subagent delegation with:
