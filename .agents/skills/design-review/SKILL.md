...
**CRITICAL: Codex subagent delegation in this skill spawns a SUBAGENT — a separate independent Codex session
...
actual Codex subagent delegation calls. A simulated review is not a specialist review.**

Issue all Codex subagent delegation calls simultaneously. Do NOT spawn one at a time.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `numbered choice prompt` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `numbered choice prompt` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `numbered choice prompt` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `numbered choice prompt`:
...
Use a third `numbered choice prompt`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `numbered choice prompt`:
