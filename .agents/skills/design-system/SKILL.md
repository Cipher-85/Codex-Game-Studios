...
Before creating or editing `design/**`, read
`.codex/instructions/path-rules/design-directory.md`. Before creating or editing
`design/gdd/**`, also read `.codex/instructions/path-rules/design-docs.md`.
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `numbered choice prompt`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `numbered choice prompt` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `numbered choice prompt` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `numbered choice prompt`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `numbered choice prompt` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `numbered choice prompt`:
...
spawn `creative-director` via Codex subagent delegation:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via Codex subagent delegation in parallel:
...
- Surface any disagreements between agents to the user via `numbered choice prompt`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via Codex subagent delegation in parallel:
...
- Present the specialists' proposals to the user for review via `numbered choice prompt`
...
For required systems: **spawn `art-director` via Codex subagent delegation** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via Codex subagent delegation using gate **CD-GDD-ALIGN** (`.codex/docs/director-gates.md`).
...
- `$design-system [next-in-order]` when another high-priority system remains.
- Fixing review findings if the GDD is in review.
...
- The main session presents the agent's output to the user via `numbered choice prompt`
- The user decides; the main session writes to file
...
2. **numbered choice prompt** at every decision point (Explain -> Capture pattern):
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, Edit, Codex subagent delegation, numbered choice prompt, TodoWrite`
- `model: sonnet`
