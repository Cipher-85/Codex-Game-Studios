---
name: hotfix
description: "Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly for Codex Game Studios."
---
...
Confirm with `numbered choice prompt`:
...
If the check passes, use `numbered choice prompt` before creating the branch:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via Codex subagent delegation with:
