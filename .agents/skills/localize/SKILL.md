---
name: localize
description: "Full localization pipeline: scan for hardcoded strings, extract and manage string tables, validate translations, generate translator briefings, run cultural/sensitivity review, manage VO localization, test RTL/platform requirements, enforce string freeze, and report coverage for Codex Game Studios."
---
...
Before writing localization string tables under `assets/data/**`, read
`.codex/instructions/path-rules/data-files.md` and create the string-table
directory lazily only after user approval.
...
Before validating string tables under `assets/data/**`, read
`.codex/instructions/path-rules/data-files.md`.
...
Spawn `localization-lead` via Codex subagent delegation. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Spawn `localization-lead` via Codex subagent delegation with:
