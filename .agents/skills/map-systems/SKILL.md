...
- Use `numbered choice prompt` to ask:
...
Then use `numbered choice prompt` to capture feedback:
...
Use `numbered choice prompt` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via Codex subagent delegation using gate TD-SYSTEM-BOUNDARY (`.codex/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `numbered choice prompt` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via Codex subagent delegation using gate PR-SCOPE (`.codex/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via Codex subagent delegation using gate CD-SYSTEMS (`.codex/docs/director-gates.md`).**
...
Use `numbered choice prompt` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `$design-system` completes, use `numbered choice prompt`:
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, Edit, numbered choice prompt, TodoWrite, Codex subagent delegation`
- `model: sonnet`
