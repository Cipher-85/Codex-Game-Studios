...
description: "Generates a comprehensive milestone progress review including feature completeness, quality metrics, risk assessment, and go/no-go recommendation. Use at milestone checkpoints or when evaluating readiness for a milestone deadline for Codex Game Studios."
---
...
Before generating the Go/No-Go recommendation, spawn `producer` via Codex subagent delegation using gate **PR-MILESTONE** (`.codex/docs/director-gates.md`).
...
If OFF TRACK, use `numbered choice prompt` before generating the recommendation:
...
If AT RISK, use `numbered choice prompt`:
