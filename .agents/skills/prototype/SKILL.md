...
Otherwise, use `numbered choice prompt` to confirm intent before proceeding:
...
Read `.codex/instructions/path-rules/prototype-code.md` before planning or
writing anything under `prototypes/**`. `prototypes/` is created lazily by this
workflow only after the user approves the implementation step.
...
Assess which path best fits the hypothesis, then use `numbered choice prompt` with your
...
Once confirmed, silently write a derived checkpoint to
`production/session-state/active.md` (create `production/session-state/` if it
does not exist). Do not ask a separate "May I write?" for this file. Include:
concept name, hypothesis, path chosen, scope bullet points, and current phase
("Phase 5 — Implement"). This lets the next session resume without starting
over if the session ends mid-build — especially important for multi-day Engine
path work.
...
If yes, create `prototypes/[concept-name]-concept/` on demand. Every file must
begin with:
...
- `full` → spawn `creative-director` via Codex subagent delegation using gate **CD-PLAYTEST** if
...
2. **Choose path** — same numbered choice prompt widget as Phase 3 (HTML / Engine / Paper).

3. **Scope** — maximum 2-3 bullet points. One mechanic, one technical question, nothing else.

4. **Build** — read `.codex/instructions/path-rules/prototype-code.md`, then
   create `prototypes/[concept-name]-spike-[date]/` on demand and follow the
   same relaxed standards as concept prototype. Hard cap: 4 hours. If not
   demonstrable in 4 hours, the question is too large. Split it.
...
7. **Silently update `production/session-state/active.md`** as a derived
   checkpoint to clear the spike and return to the current sprint state. Do not
...
## Codex Worktree Guidance

Before editing implementation files for this workflow, verify whether the current workspace is isolated. If not, create or request an isolated worktree before making prototype or vertical-slice changes, then run the relevant validation before merging results back.
