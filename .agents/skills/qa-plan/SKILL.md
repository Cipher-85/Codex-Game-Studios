...
**Argument:** `$ARGUMENTS` (blank = ask user via numbered choice prompt)
...
- **No argument** — use `numbered choice prompt`:
...
then ask two questions together using `numbered choice prompt`:
...
- Use `numbered choice prompt` for scope selection when no argument is provided.
