---
name: quick-design
description: "Lightweight design spec for small changes — tuning adjustments, minor mechanics, balance tweaks. Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files for Codex Game Studios."
---
...
Before writing under `design/**`, read
`.codex/instructions/path-rules/design-directory.md`. For tuning changes that
read or propose updates to `assets/data/**`, also read
...
Present the inferred classification using `numbered choice prompt`:
...
Present the draft to the user in full. Then use `numbered choice prompt`:
