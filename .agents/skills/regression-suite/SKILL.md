...
description: "Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate for Codex Game Studios."
---
...
Before reading or writing `tests/**`, read
`.codex/instructions/path-rules/test-standards.md`. If `tests/` does not exist,
report that the test framework has not been scaffolded and recommend
`$test-setup`; create `tests/regression-suite.md` only after explicit user
approval.
...
- `$regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `numbered choice prompt`:
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, Edit, numbered choice prompt`
- `model: sonnet`
