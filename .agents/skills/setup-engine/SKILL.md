---
name: setup-engine
description: "Configure the project's game engine and version. Pins the engine in Codex project instructions, detects knowledge gaps, and populates engine reference docs when the version is beyond the model's training data."
---
...
**Question 1 — Prior experience** (ask this first, always, via `numbered choice prompt`):
...
**Question 2 — Target platform** (ask this second, always, via `numbered choice prompt` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `numbered choice prompt` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `numbered choice prompt` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use `numbered choice prompt`:
