---
name: skill-test
description: "Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report) for Codex Game Studios."
---
...
Use `CCGS Skill Testing Framework/` as the canonical test-spec and fixture source for this workflow. The upstream framework is missing a `vertical-slice` skill spec; report that as a known upstream gap.
