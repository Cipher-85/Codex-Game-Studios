---
name: studio-next
description: "Lightweight Codex Game Studios continuity router. Reads handoff, session, sprint, stage, workflow, and slice state to list viable next actions with one recommended choice after a work unit."
---
...
5. `.codex/docs/workflow-catalog.yaml`
6. `src/README.md`
7. `production/qa/smoke-*.md` and `production/qa/qa-signoff-*.md`, latest only
8. `.agents/skills/*/SKILL.md`, frontmatter names only, to know which routed
...
- Phase next step from `.codex/docs/workflow-catalog.yaml`.
...
When several next actions are viable, use `request_user_input` when available:
...
If `request_user_input` is unavailable, fall back to a concise numbered prompt
...
and exists to preserve post-task continuity in Codex surfaces.
