---
name: team-level
description: "Orchestrate level design team: level-designer + narrative-director + world-builder + art-director + systems-designer + qa-tester for complete area/level creation for Codex Game Studios."
---

When this skill is invoked:

**Decision Points:** At each step transition, use `numbered choice prompt` to present
...
Spawn all three agents simultaneously — issue all three Codex subagent delegation calls before waiting for any result.
...
**Gate**: Use `numbered choice prompt` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `numbered choice prompt` with options:
...
**Gate**: Use `numbered choice prompt` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `numbered choice prompt` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `numbered choice prompt` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all subagent outputs are collected, spawn `level-designer` via Codex subagent delegation to compile and write the final document:
...
to sub-agents spawned via Codex subagent delegation. Each sub-agent enforces the "May I write to [path]?"
...
- `user-invocable: true`
- `allowed-tools: Read, Glob, Grep, Write, Edit, Bash, Codex subagent delegation, numbered choice prompt, TodoWrite`
- `model: sonnet`
