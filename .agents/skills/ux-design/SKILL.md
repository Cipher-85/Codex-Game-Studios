...
Before creating or editing UX specs under `design/**`, read
`.codex/instructions/path-rules/design-directory.md`.
...
**If no argument is provided**, do not fail — ask instead. Use `numbered choice prompt`:
...
2. **Questions**: Ask what is needed to draft this section. Use `numbered choice prompt`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `numbered choice prompt` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `numbered choice prompt`:
...
7. **Write**: Use `numbered choice prompt`: "May I write the [section name] section to `[filepath]`?"
...
- Use `numbered choice prompt` to capture the choice:
...
- Use `numbered choice prompt`: "Want an ASCII wireframe as part of this spec?"
...
Use `numbered choice prompt` to confirm:
...
Use `numbered choice prompt` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `numbered choice prompt`:
...
Then use `numbered choice prompt`:
