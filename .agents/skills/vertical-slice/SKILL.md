...
writing the slice under `prototypes/**`. During implementation, also read
`.codex/instructions/path-rules/source-code.md` plus the relevant source
subtree rule (`gameplay-code.md`, `engine-code.md`, `ai-code.md`,
`network-code.md`, or `ui-code.md`) for any source-shaped files produced inside
the slice.
...
concept name, validation question, systems in scope, art quality level, and
current phase ("Phase 4 — Implement"). Silently update this file at the end of
each build day with what was completed. This is the primary recovery mechanism
if the session ends mid-slice — multi-week Engine builds will span many
sessions.
...
If yes, create `prototypes/[concept-name]-vertical-slice/` on demand. Every file
must begin with:
...
- `full` → spawn `creative-director` via Codex subagent delegation using gate **CD-PLAYTEST**
