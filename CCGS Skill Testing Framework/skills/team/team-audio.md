...
`numbered choice prompt` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] `numbered choice prompt` is used at step transitions before proceeding
...
3. `numbered choice prompt` presents audio direction; user approves before Step 2 begins
...
5. `numbered choice prompt` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
7. `numbered choice prompt` presents technical plan; user approves before Step 4 begins
...
- [ ] `numbered choice prompt` appears after Step 1 output and before Step 2 launch
...
- [ ] gameplay-programmer is not launched until Step 3 `numbered choice prompt` is approved
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `numbered choice prompt`
4. `numbered choice prompt` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `numbered choice prompt` surfaces the gap before Step 3 is launched
...
- [ ] `numbered choice prompt` is used after every step output before the next step launches
