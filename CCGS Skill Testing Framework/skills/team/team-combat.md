...
(parallel) → Integration → Validation → Sign-off. Uses `numbered choice prompt` at each
...
2. Phase 2 — gameplay-programmer + ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `numbered choice prompt` presented with architecture options before Phase 3 begins
...
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `numbered choice prompt` confirms integration before Phase 5
...
**Assertions:**
- [ ] `numbered choice prompt` called at each phase gate (at minimum before Phase 3 and before Phase 5)
...
4. `numbered choice prompt` presented with options: (a) Skip ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `$architecture-decision` first
...
6. `numbered choice prompt` includes engine specialist's notes alongside the architecture sketch
