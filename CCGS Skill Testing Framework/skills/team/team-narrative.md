...
and world-builder in parallel). Uses `numbered choice prompt` at each phase transition to
...
- [ ] `numbered choice prompt` is used at phase transitions before proceeding
...
2. `numbered choice prompt` presents the narrative brief; user approves before Phase 2 begins
...
4. `numbered choice prompt` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `numbered choice prompt` presents level narrative plan; user approves before Phase 4 begins
...
8. `numbered choice prompt` presents review results; user approves before Phase 5 begins
...
- [ ] `numbered choice prompt` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer Task calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `numbered choice prompt` is approved
...
6. `numbered choice prompt` presents options:
...
- [ ] `numbered choice prompt` presents at least 3 options including "stop and resolve first"
...
- [ ] No `numbered choice prompt` is used — output is direct guidance
...
5. `numbered choice prompt` presents options:
...
- [ ] `numbered choice prompt` offers the option to fix now vs. flag and proceed
...
- [ ] `numbered choice prompt` offers at least one option to resolve the missing prerequisite
...
- [ ] `numbered choice prompt` is used after every phase output before the next phase launches
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `numbered choice prompt`
