...
Uses `numbered choice prompt` at each phase transition. Delegates all file writes to
...
- [ ] Uses `numbered choice prompt` at phase transitions for user approval before proceeding
...
2. Phase 1b — `$ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `numbered choice prompt` confirms spec before review
3. Phase 1c — `$ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `numbered choice prompt` confirms before Phase 3
...
4. `numbered choice prompt` presented with the specific flagged concerns and options:
...
- [ ] `numbered choice prompt` presents the specific flagged concerns before offering options
...
5. `numbered choice prompt` presents all three review results before Phase 5 begins
...
- [ ] `numbered choice prompt` shows all three review streams' results together before Phase 5 approval
...
3. `numbered choice prompt` presented with options:
...
- [ ] `numbered choice prompt` offers a "create pattern library first" option (referencing `$ux-design patterns`)
...
- [ ] `numbered choice prompt` used at each phase transition — user approves before pipeline advances
