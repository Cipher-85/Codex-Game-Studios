...
| `.claude/statusline.sh` | Always-on context/model/stage/session breadcrumb | Claude scriptable status line command | Codex native footer config plus startup/on-demand studio status | `.codex/config.toml` `[tui].status_line`, `.codex/hooks/studio-status-on-start.sh`, `.agents/skills/studio-status/SKILL.md` | replaced | partial | Preserve context/model visibility with Codex `status_line`. Preserve stage/review/session breadcrumb through shared-state parsing in `studio-status-on-start.sh` and `studio-status`; no verified Codex project script footer item for the full game breadcrumb. | Config parse; status_line item allowlist; hook fixture; `studio-status` smoke reads `production/session-state/active.md`. |
...
| `CCGS Skill Testing Framework/skills/**/<skill>.md` | 72 | same relative tree under target root | ported | semantic equivalent | Specs exist for all upstream skills except `vertical-slice`. | Skill-spec vs 73-skill manifest diff; expected missing `vertical-slice`. |
| Missing `vertical-slice` skill spec | 1 expected spec absent upstream | optional new `CCGS Skill Testing Framework/skills/utility/vertical-slice.md` | blocked unless generated as new Codex artifact | blocked | Upstream gap, not a Codex conversion miss. Phase 5 may add a new Codex-specific spec after labeling it new. | Validation reports exactly one known missing upstream spec. |
...
- 40 copied templates.
- 127 testing-framework assets with exactly one known missing upstream skill spec: `vertical-slice`.
