...
| Permission profiles | `.codex/config.toml` with `default_permissions` and `[permissions.<name>]` | Select one complete native profile without a project-local `approval_policy` or legacy sandbox keys. |
...
| `.claude/settings.json` permissions | Allow/deny command and path policy | Claude permissions in settings JSON | Complete Codex permission profile plus command rules | `.codex/config.toml`, `.codex/rules/settings.rules` | replaced | semantic equivalent | Select `[permissions.game_studios] extends = ":workspace"` through `default_permissions`, restore writes to `.git`, `.agents`, and `.codex`, deny sensitive env paths, and allow only `github.com`; command policies remain in `.rules`. Do not set project-local `approval_policy`: the completed profile must support normal handoff without escalation. | TOML parse; rules lint; forbidden command fixture; fresh trusted session proves the profile is active with Git/runtime writes, `.env*` denials, and GitHub network access; end-to-end handoff proves commit and push. |
